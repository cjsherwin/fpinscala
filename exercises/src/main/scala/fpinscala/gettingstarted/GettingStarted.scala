...
object MyModule {
  ...
  def fib(n: Int): Int = {
    @tailrec
    def go(c: Int, i1: Int, i2: Int): Int = {
      println(s"$c: $i1, $i2")
      if (c == 0) i1
      else go(c - 1, i2, i1 + i2)
    }
    go(n, 0, 1)
  }
...
object PolymorphicFunctions {
  ...
  def isSorted[A](as: Array[A], gt: (A,A) => Boolean): Boolean = {
    as.sliding(2).forall(a => gt(a.head, a.last))

    @tailrec
    def loop(c: Int): Boolean = {
       if (c >= as.length - 1) true
       else if (gt(as(c), as(c+1))) false
       else loop(c + 1)
    }
    loop(0)
  }
